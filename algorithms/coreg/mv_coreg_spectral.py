...
from .my_eig import myeigh_v1, myeigh_v2

AFFINITY_METRICS = ['rbf', 'nearest_neighbors', 'poly', 'no']
...
class MultiviewCoRegSpectralClustering(MultiviewSpectralClustering):
    ...
    def __init__(self, n_clusters=2, v_lambda=2, random_state=None,
                 info_view=None, max_iter=10, n_init=10, affinity='no',
                 gamma=1, n_neighbors=10):
    ...
    def _init_umat(self, X):
        ...
        if np.linalg.det(d_mat) == 0:
            d_alt = np.sqrt(np.linalg.pinv(d_mat))
        else:  # numpy.linalg.LinAlgError: Singular matrix
        ...
        u_mat, d_mat = self.optSolver(laplacian)

        obj_val = np.sum(d_mat)

        return u_mat, laplacian, obj_val

    def optSolver(self, X):
        # try:
        #     d, u = myeigh_v1(X)
        ...
        #     d, u = myeigh_v2(X)
        # finally:
        #     u_mat = u[:, 0:self.n_clusters]
        #     d_mat = d[0:self.n_clusters]
    ...
    def fit(self, Xs):
        ...
        for it in range(1, self.max_iter):
            ...
            for v1 in range(1, self._n_views):
                ...
                U_mats[v1], d_mat = self.optSolver(l_mat)
            ...
            U_mats[0], d_mat = self.optSolver(l_mat)
